...
# --- MSRNet Model & Components (R2AttU-Net) ---
...
class RRCNN_block(nn.Module):
    ...
    def forward(self, x):
        residual = self.adapt(x); out = self.rcnn1(residual); out = self.rcnn2(out)
        return out + residual
...
class AttentionGate(nn.Module):
    def __init__(self, F_g, F_l, F_int):
        ...
        self.W_g = nn.Sequential(nn.Conv2d(F_g, F_int, kernel_size=1, bias=False), nn.GroupNorm(num_groups_int, F_int))
        self.W_x = nn.Sequential(nn.Conv2d(F_l, F_int, kernel_size=1, bias=False), nn.GroupNorm(num_groups_int, F_int))
...
class MSRNDown(nn.Module):
    def __init__(self, in_channels, out_channels, t=2):
        super().__init__()
        self.pool = nn.MaxPool2d(2); self.rrcnn = RRCNN_block(in_channels, out_channels, t=t)
    def forward(self, x): return self.rrcnn(self.pool(x))
...
class AttentionUp(nn.Module):
    def __init__(self, in_channels, skip_channels, out_channels, bilinear=True, t=2):
        ...
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            conv_in_channels = in_channels + skip_channels; gate_channels = in_channels
        else:
            up_out_channels = in_channels // 2
            self.up = nn.ConvTranspose2d(in_channels, up_out_channels, kernel_size=2, stride=2)
            conv_in_channels = up_out_channels + skip_channels; gate_channels = up_out_channels
    ...
    def forward(self, x1, x2):
        ...
        if diffY != 0 or diffX != 0:
            pad_left = diffX // 2; pad_right = diffX - pad_left; pad_top = diffY // 2; pad_bottom = diffY - pad_top
            x1 = nn.functional.pad(x1, [pad_left, pad_right, pad_top, pad_bottom])
...
class MSRNet(nn.Module):
    def __init__(self, n_ch=3, n_cls=3, hidden_size=64, bilinear=True, t=2):
        super().__init__()
        if n_ch <= 0 or n_cls <= 0 or hidden_size <= 0: raise ValueError("n_ch, n_cls, hidden_size must be positive")
        self.n_ch, self.n_cls, self.hidden_size, self.bilinear, self.t = n_ch, n_cls, hidden_size, bilinear, t
        h = hidden_size
        chs = {'enc1': max(1, h), 'enc2': max(1, h*2), 'enc3': max(1, h*4), 'enc4': max(1, h*8), 'bottle': max(1, h*16)}
        self.inc = RRCNN_block(n_ch, chs['enc1'], t=t)
        self.down1 = MSRNDown(chs['enc1'], chs['enc2'], t=t); self.down2 = MSRNDown(chs['enc2'], chs['enc3'], t=t)
        self.down3 = MSRNDown(chs['enc3'], chs['enc4'], t=t); self.down4 = MSRNDown(chs['enc4'], chs['bottle'], t=t)
...
def load_model_for_conversion(checkpoint_path, device='cpu'):
    ...
    logging.info(f"Instantiating {model_type_saved.upper()} with effective_hidden_size={effective_model_size}, bilinear={bilinear_mode}")
    try:
        base_model = create_model(
            model_type=model_type_saved,
            n_ch=3, n_cls=3,
            hidden_size=effective_model_size,
            bilinear=bilinear_mode,
            t=recurrence_t
        )
    except Exception as e:
        logging.error(f"Failed to instantiate {model_type_saved.upper()} model: {e}", exc_info=True)
        raise RuntimeError(f"Model instantiation failed with effective size {effective_model_size}") from e
