...
from .helpers import collate_skip_none


def save_previews(model, fixed_src_batch, fixed_dst_batch, output_dir,
                  current_epoch, global_step, device, preview_save_count,
                  preview_refresh_rate, fixed_mask_batch=None,
                  use_mask_input=False, use_bce_dice=False, use_amp=False,
                  use_auto_mask=False, fixed_auto_mask_batch=None,
                  auto_mask_gamma=1.0, diff_amplify=5.0):
    ...
    try:
        ...
        logging.error(f"Preview inference error (Step {global_step}): {e}")
        model.train()
        return
    model.train()
    ...
    auto_mask_vis = None
    if has_auto_mask:
        auto_mask_refined = refine_auto_mask(fixed_auto_mask_batch[:num_samples], gamma=auto_mask_gamma)
    ...
    if has_mask:
        ...
        row = lambda i: [src_denorm[i], mask_3ch[i], dst_denorm[i], pred_denorm[i],
                         diff_heatmap(src_denorm[i], dst_denorm[i], amplify=diff_amplify)] + (
    ...
    try:
        ...
        logging.error(f"Preview grid error (Step {global_step}): {e}")
        return
    try:
        img_pil = T.functional.to_pil_image(grid)
    except Exception as e:
        logging.error(f"Preview PIL convert error (Step {global_step}): {e}")
        return
    ...
    try:
        ...
        logging.log(log_level, f"Saved preview ({num_samples} samples) (E{current_epoch + 1}, S{global_step}, #{preview_save_count})"
...
def save_val_previews(model, fixed_src_batch, fixed_dst_batch, output_dir,
                      current_epoch, global_step, device, use_bce_dice=False,
                      use_amp=False, use_auto_mask=False, use_mask_input=False,
                      diff_amplify=5.0):
    ...
    try:
        ...
        logging.error(f"Val preview inference error (Step {global_step}): {e}")
        model.train()
        return
    model.train()
    src_denorm = denormalize(src_select)
    ...
    if has_dst:
        ...
        row = lambda i: [src_denorm[i], pred_denorm[i],
                         diff_heatmap(src_denorm[i], pred_denorm[i], amplify=diff_amplify)]
        nrow = 3
    combined = [item for i in range(num_samples) for item in row(i)]
    if not combined:
        return
