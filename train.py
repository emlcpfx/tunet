...
class RecurrentConv(nn.Module):
    """Single conv block applied recurrently t times with weight sharing."""
...
class RRCNN_block(nn.Module):
    """Recurrent Residual Convolutional block — replaces DoubleConv for MSRNet."""
...
class AttentionUp(nn.Module):
    ...
    def __init__(self, in_channels, skip_channels, out_channels, bilinear=True, t=2):
        ...
        self.rrcnn = RRCNN_block(conv_in_channels, out_channels, t=t)
    def forward(self, x1, x2):
        x1 = self.up(x1)
        diffY = x2.size(2) - x1.size(2)
        diffX = x2.size(3) - x1.size(3)
...
class MSRNet(nn.Module):
    """Multi-Scale Recurrent Network (R2AttU-Net architecture)."""
