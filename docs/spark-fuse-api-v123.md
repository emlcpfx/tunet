# Spark Fuse API — customer guide (v1.23)
...
Auth is a Spark bearer token. You get one from `POST /auth/login`
once and reuse it for every API call until it expires (tokens are valid
for 30 days — see [§1.3](#13-token-lifetime--refresh)). The same token
...
Spark issues bearer tokens through `POST /auth/login`. You
...
`/auth/login` is the only supported auth surface and the only one
...
```bash
export SPARK_TOKEN=$(curl -sX POST "$SPARK_HOST/auth/login" \
...
Bearer tokens are valid for **30 days**. The token carries the
`offline_access` claim and is issued with a 30-day lifetime, so a single
token comfortably covers a long working session against both the Spark
API and ShareSync. When yours finally expires you'll get an HTTP 401
back from a `/api/compute/...` call; re-call `POST /auth/login` with the
same credentials to mint a new one.
...
#### Long uploads vs. token expiry

Auth on a ShareSync WebDAV request is validated **once, at the moment
the request starts** (the OIDC proxy checks the bearer when it
accepts the request, not continuously as the body streams). The
practical consequence for large uploads:

- A `PUT` that **begins** with a still-valid token **completes** even if
  the transfer runs for hours and crosses the token's expiry — the
  connection that started under a valid token isn't torn down
  mid-stream.
- A `PUT` that **starts** with an already-expired token is rejected
  immediately with `401` — there is no mid-upload re-auth or refresh.

So for a multi-GB upload, **mint a fresh token shortly before you start
the transfer** (a 30-day token started today will not expire mid-upload,
but minting right before a very long transfer removes any doubt). If a
`PUT` does come back `401`, re-login and restart that upload.
...
| ---- | ------------------ | ---------------- | ------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| 200  | `/auth/login`  | `success: false` | Login failed                    | Wrong email/password, account not email-verified, account disabled, or upstream identity failure. Check `resp`. |
| 401  | `/api/compute/...` | —                | Token expired or invalid        | Re-call `POST /auth/login` and retry the request.                                                           |
...
```json
{
  ...
  "outputShareSyncPath": "/Spark Fuse Job Outputs/16fb6d80-0ac3-4d42-9c5d-80c02a26e5ae/",
  "createdAt": "2026-05-05T09:12:32.338757+00:00",
  "output": {
    "shareSyncPath": "/Spark Fuse Job Outputs/16fb6d80-0ac3-4d42-9c5d-80c02a26e5ae/",
    "shareSyncSpaceName": null,
    "shareSyncBaseUrl": "https://your-org.files.sparkcloud.studio/dav/spaces/<id>/Spark%20Fuse%20Job%20Outputs/16fb6d80-.../"
  },
  "input": null
}
```
...
| -------------------------- | -------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `image`                    | string   | yes      | Container image reference (with optional tag or digest). Any **public** `linux/amd64` image works regardless of registry (Docker Hub, GHCR, Quay, public ECR, `gcr.io`, etc.) — see [§2.5](#25-using-your-own-docker-image). **Private** repositories are supported on Docker Hub (`docker.io`) and GHCR (`ghcr.io`) via `registryCredentialName` ([§2.6](#26-private-container-images)); private repos on other registries are not supported in v1. Pinning by digest is recommended for reproducibility. |
...
| `outputShareSyncPath`      | string   | no       | ShareSync logical path that outputs are written to. Defaults to `/Spark Fuse Job Outputs/{jobId}/`.                                                                                                                                                                                                                                                                                                                                   |
| `outputShareSyncSpaceName` | string   | no       | Name of a ShareSync Project to write outputs into. Omit to use your Personal space. Spark Fuse validates the project exists at submission time and rejects with HTTP 400 if it doesn't.                                                                                                                                                                                                                                  |
...
| `inputShareSyncSpaceName`  | string   | no       | Name of the ShareSync Project containing your input path (or, with `inputPushMode='auto-prepare'`, the Project Spark Fuse will allocate the input folder under). Omit to use your Personal space.                                                                                                                                                                                                                                  |
...
| `assetsShareSyncPath`      | string   | no       | A ShareSync path mounted **read-only and lazily** at `/assets` for a large, stable library reused across runs (model weights, datasets). Only the bytes you read are pulled, and the library is cached on the compute node across jobs. PROPFIND-probed at submit (HTTP 400 if unreachable). Independent of, and combinable with, `inputShareSyncPath`. See [§3.5](#35-read-only-asset-library-assets). |
| `assetsShareSyncSpaceName` | string   | no       | Name of the ShareSync Project containing `assetsShareSyncPath`. Omit to use your Personal space. |
...
| `imageAffinity`            | string   | no       | (`mode='instant'` only) Cached-image placement preference, for large images where the cold `docker pull` dominates start time. Both values **softly** steer your job onto a warm node that **already has your image cached** when one is free; neither ever fails or delays the job — if none is free it places immediately on a fresh node and cold-pulls (seeding the cache for next time). `'preferred'` (default) does this silently. `'required'` does the same placement but **notifies** you when it had to cold-pull instead of giving a cache hit (via `imageCacheHit` on the job status + the `compute.job.provisioning` webhook + a log line). Any other value is rejected with HTTP 400. Echoed back on every job response as `imageAffinity` (snake_case alias `image_affinity`). See [§2.7](#27-cached-image-affinity-faster-warm-starts). |
...
| `registryCredentialName`   | string   | no       | Name of a credential created via `POST /api/compute/credentials/registry` ([§2.6](#26-private-container-images)). Required when `image` references a private Docker Hub or GHCR repository; omit for public images (any registry). The credential's `registryHost` must match the image's host. Submission-time errors use codes `registry_credential_required`, `credential_not_found`, `credential_type_mismatch`, `credential_registry_mismatch`, `credential_validation_failed`, or `unsupported_registry_in_image`. |
...
The `compute.job.provisioning` event additionally carries the cached-image
affinity outcome (§2.7) so you learn at placement time whether the job got a
warm cache hit or a cold pull on a fresh node:

```json
{
  "event": "compute.job.provisioning",
  "jobId": "09da25d2-0a45-4cbb-bc24-717a0e7eb300",
  "status": "provisioning",
...
`imageCacheHit: false` with `imageAffinity: "required"` is the "image wasn't
cached anywhere free — we're pulling it fresh on a new node for you (and it'll
be cached after this run)" signal. `imageCacheHit` is `null` when the job
carried no resolvable image digest.
...
| 400 `Output ShareSync project ... not found`                      | The project name you supplied doesn't exist or you don't have write access.                                                                                               |
...
| 400 `imageAffinity must be 'preferred' or 'required'`             | `imageAffinity` was supplied with a value other than `'preferred'` or `'required'`. Omit the field to use the `'preferred'` default. See [§2.7](#27-cached-image-affinity-faster-warm-starts).                                                                                                                                                                                                                                                                  |
| 400 `Image '…' not found in the registry`                         | The `image` reference couldn't be resolved at submit time. Verify the reference (`registry/repo:tag` or `@digest`) and that the repo is public (or that you passed `registryCredentialName` for a private Docker Hub / GHCR repo — see [§2.6](#26-private-container-images)). |
| 400 `Image '…' has no linux/amd64 manifest`                       | The image resolved but provides no `linux/amd64` entry. Spark Fuse v1 is `linux/amd64`-only — rebuild with `--platform=linux/amd64` and re-push (see [§2.5](#25-using-your-own-docker-image)). |
| 400 `Image '…' could not be reached over the network`            | Spark couldn't reach the registry while validating the manifest (validation is time-bounded, so this fails fast rather than hanging). Check the registry hostname; if the registry is healthy, retry — it may be a transient blip. |
...
2. **Push to a registry Spark can pull.** Any **public** registry that
   allows anonymous `docker pull` works — Docker Hub, GHCR, Quay,
   public ECR, `gcr.io`, etc. For **private** repositories, Spark
   currently supports Docker Hub (`docker.io`) and GitHub Container
   Registry (`ghcr.io`); store a token via the credentials API and
   reference it on submission with `registryCredentialName` (see
   [§2.6 Private container images](#26-private-container-images)).
   Authenticated pulls from other registries are not supported in v1
   — let Spark know if you need one.
...
### 2.6 Private container images

Private repositories on **Docker Hub** (`docker.io`) and **GitHub
Container Registry** (`ghcr.io`) are supported via per-organisation
credentials. The flow is:

1. **Mint a registry token.** For Docker Hub use a [Personal Access
   Token](https://hub.docker.com/settings/security) (not your account
   password). For GHCR use a GitHub Personal Access Token with package
   read access — a **classic** PAT with the **`read:packages`** scope is
   the simplest and most reliable (it's the same token you use to
   `docker login ghcr.io` and push the image; add `write:packages` if you
   also push from this token). A **fine-grained** PAT with **`Packages:
   Read`** permission on the relevant repositories also works. The token
   is what you pass as `secret` below; for GHCR `username` is your GitHub
   username (lowercase — GHCR namespaces are lowercase).

2. **Store the credential in Spark.** Org admins only. The secret is
   encrypted at rest and never returned by any endpoint after
   creation; rotate it later via `PUT /:credentialId/secret`. Spark
   probes the registry immediately and records the result on the
   credential row (`lastValidationStatus`).

   ```bash
   curl -sX POST "$SPARK_HOST/api/compute/credentials/registry" \
     -H "Authorization: Bearer $SPARK_TOKEN" \
     -H "Content-Type: application/json" \
     -d '{
       "name": "mycorp-ghcr",
   ...
   Response (the `name` is what you reference at submission time;
   `lastValidationStatus='success'` means the token was accepted by
   the registry):
   ...
   ```json
   {
     ...
     "lastValidationAt": "2026-05-25T07:54:12.341Z",
     "lastValidationError": null,
     "lastUsedAt": null,
     "createdAt": "2026-05-25T07:54:12.341Z",
     "updatedAt": "2026-05-25T07:54:12.341Z",
     "createdByUserId": 12
   }
   ```

3. **Reference the credential on submission.** Pass `registryCredentialName`
   alongside `image`. Spark looks up the credential in your
   organisation, confirms the host matches the image's registry,
   runs an authenticated manifest probe (so private images get the
   same submission-time arch check public images get), and persists
   the credential reference on the job row.
   ...
   curl -sX POST "$SPARK_HOST/api/compute/jobs" \
     ...
     -H "Content-Type: application/json" \
     -d '{
       "image": "ghcr.io/mycorp/training-runner:v3.2.1",
       "command": ["python3", "/app/train.py"],
       "instanceType": "g7e.2xlarge",
       "registryCredentialName": "mycorp-ghcr"
     }' | jq '.'
   ```

4. **At runtime** the compute agent runs `docker login --password-stdin`
   with the decrypted credential before `docker pull`, then
   `docker logout` once the pull completes. The secret never appears
   on the command line, in environment variables, or in container
   logs. On a successful pull the agent posts a fire-and-forget
   callback that bumps `lastUsedAt` and writes a `used_in_job`
   audit row.
...
| Endpoint | Behaviour |
|----------|-----------|
| `POST /api/compute/credentials/registry` | Create. Validates the credential against the registry; the row is saved even on a failed probe so you can fix a typo via `PUT /:id/secret` without re-entering everything. |
| `GET /api/compute/credentials` | List your organisation's credentials. Filter with `?credentialType=registry&registryHost=ghcr.io`. |
| `PUT /api/compute/credentials/:credentialId/secret` | Rotate the secret in place. `name`, `registryHost`, and `username` are immutable — delete and re-create to change them. The new secret is re-probed against the registry. |
| `DELETE /api/compute/credentials/:credentialId` | Hard delete. Future submissions referencing this credential by name fail with `error_code='credential_not_found'`; jobs that already had the credential included in their jobSpec at the time of deletion continue to use that snapshot. |
| `GET /api/compute/credentials/:credentialId/audit` | Audit trail. Records `validated` (on create / rotate), `updated` (on rotate), `used_in_job` (per job after the pull succeeds), and `deleted`. |

#### Submission-time errors specific to credentials

| `error_code` | When you'll see it |
|---|---|
| `registry_credential_required` | Image is private but you didn't pass `registryCredentialName`. |
| `credential_not_found` | `registryCredentialName` doesn't match any credential in your organisation. |
| `credential_type_mismatch` | The named credential exists but isn't of type `registry`. |
| `credential_registry_mismatch` | The credential's `registryHost` doesn't match the image's host (e.g. you referenced a Docker Hub credential for a `ghcr.io/...` image). |
| `credential_validation_failed` | Spark could authenticate to the registry but the token was rejected when probing this specific image. Most common cause is an expired token — rotate via `PUT /:id/secret`. |
| `unsupported_registry_in_image` | `registryCredentialName` was supplied but the image targets a registry Spark doesn't have a credential path for (anything other than `docker.io` or `ghcr.io`). Public images on other registries continue to work without `registryCredentialName`. |
| `invalid_registry_credential_name` | `registryCredentialName` was supplied but is empty / not a string. |

If the credential is deleted **after** submission but before the agent
provisions the container, the job terminates with `status='failed'`
`error_code='credential_not_found'` on the next provision attempt. This
is deliberate — your name-based reference always reflects the current
state of the credential, so revoking a leaked token instantly stops
new jobs from using it.

#### Security model

* Secrets are encrypted at rest with AES-256-CBC using a dedicated
  key (`COMPUTE_CREDENTIALS_ENCRYPT_KEY`) that is **separate** from
  every other secret-encryption key on the platform. Spark Cloud
  Studio operators do not have a path to retrieve the plaintext via
  the API — `GET` endpoints never include the secret in any
  response.
* The Docker daemon's credential cache is dropped after every pull
  (`docker logout` runs in a `finally`), so a subsequent job that
  lands on the same warm-pool instance starts from a clean state.
* Use a **token**, not your account password. Tokens are revocable
  per-credential without affecting your interactive registry login.
* Rotate often. `PUT /:credentialId/secret` updates the secret in
  place without changing the credential's name, so existing job
...
### 2.7 Cached-image affinity (faster warm starts)

Large container images (a ComfyUI or Blender+CUDA image can be **10–20 GB**)
spend most of their cold-start time in the `docker pull`, not in
provisioning. Spark Fuse warm-pool nodes **cache images they've pulled
before**, so a node that already ran your image can start your next job
without re-pulling. Cached-image affinity lets you ask the scheduler to
**land your job on such a node**.

This is distinct from `idleHoldSeconds` (§12): the idle-hold keeps *one
specific* instance warm for *your org's* immediate next job. Cached-image
affinity is broader — it steers placement toward **any** warm node in the
pool that has your image cached, even across the whole pool and after the
hold has expired.

**Affinity never fails or delays your job.** Both modes are a *soft*
preference: when a free warm node already has your image cached, the
scheduler steers your job onto it; when none is free, your job places
immediately on a fresh node and cold-pulls (and that node is then cached for
your next run). The only difference between the two values is **notification**
— `'required'` tells us you care, so we surface a clear signal when we had to
cold-pull instead of giving you a cache hit.

| `imageAffinity`  | Behavior |
| ---------------- | -------- |
| `'preferred'` (default) | **Soft, silent.** When a free warm node already has your image cached, the scheduler prefers it (skipping a multi-GB pull outweighs any other placement preference). When none is free, your job places immediately on a fresh node and cold-pulls — **no penalty, no waiting**. On for every job by default; you don't need to do anything. |
| `'required'`     | **Soft, with notification.** Same placement as `'preferred'` — it **never** hard-fails and **never** holds your job. The difference: when no cached node is available and we cold-pull on a fresh node, we tell you (see *How you're notified* below) so you know this run is a cold start and the image is now warming for next time. Use this for repeated runs of a large image where you want visibility into cache hits vs. misses. |

```jsonc
// Repeated ComfyUI runs on a big image — prefer a cached node and get
// notified when we have to cold-pull instead:
{
  "image": "docker.io/yourorg/comfyui@sha256:abcd...",
...
**How you're notified (the `imageCacheHit` signal).** Once your job is placed,
Spark Fuse records whether it got a cache hit and surfaces it on three
channels:

- **Job status** — `GET /api/compute/jobs/{jobId}` returns `imageCacheHit`:
  `true` (landed on a node that already had the image — fast start), `false`
  (cold pull on a fresh node — slower this once, now cached), or `null`
  (not yet placed, i.e. still `queued`).
- **Webhook** — the `compute.job.provisioning` event payload includes both
  `imageCacheHit` and `imageAffinity`, so a `'required'` job with
  `imageCacheHit: false` is your "we're pulling fresh for you" signal without
  a follow-up GET.
- **Live log** — your job's log stream contains an explicit line at start,
  e.g. *"Image not cached on this node — pulling it now. This first run is
  slower; the image will be cached on the warm pool for your next job."*

The **first run** of a brand-new image naturally reports `imageCacheHit:
false` everywhere (it isn't cached anywhere yet); after it completes, the node
that ran it carries the image and subsequent jobs can hit the cache.

Notes:

- **Always pin by digest** (`image@sha256:...`) for the most reliable cache
  matching and reproducibility. Affinity is keyed on the resolved image
  digest, not a mutable tag.
- The cache is best-effort: if a node has since cleared your image to free
  up space, it simply drops out of the cached set, so both modes quietly
  degrade to a (correct) cold pull — you never get a *wrong* placement.
- `imageAffinity` is echoed back on every job response (snake_case alias
  `image_affinity`); `imageCacheHit` appears once the job is placed.

---

## 3. Uploading input data

Spark Compute exposes `/input/` (read-only) inside your container if — and
only if — you've supplied input data. There are **two distinct ways** to
get data into `/input/`, and they are easy to confuse, so here is the
contrast up front:

| | **Push workflow** (§3.1) | **Mount workflow** (§3.2) |
|---|---|---|
| Triggered by | `inputPushMode: "auto-prepare"` | `inputShareSyncPath: "<path>"` |
| Where your data starts | Nowhere yet — on your local machine | Already staged in a ShareSync folder |
| How data moves | You **`PUT` a tarball after submit**; Spark Fuse waits for it, extracts it into `/input/` | Spark Fuse **pulls the existing path** into `/input/` at job start — no upload step |
| Timing risk | Upload races a wait window on a billing instance ([§3.1](#31-push-workflow-auto-prepare)) | None — the data is already there before the job runs |
| Best for | Small, per-job inputs (a few MB of source) | Large or reused data — model libraries, datasets, anything you don't want to re-upload every run |

Two things to keep clear about the naming:

- **"Push" / `auto-prepare`** means *you push data up after submitting* —
  the server allocates an empty folder and hands you a one-shot upload
  command. Nothing exists in ShareSync until your `PUT` lands.
- **"Mount"** means *you point at data that already exists* in ShareSync.
  Despite the name, today this is **not** a live/lazy filesystem mount:
  at job start Spark Fuse **copies** everything under your path into a
  fresh read-only `/input/` for that job. The data is reused across jobs
  on the ShareSync side (you don't re-upload it), but each job still
  pulls a fresh copy server-side.

If you supply neither field, `/input/` is not present at all (§3.3).
...
```json
{
  ...
  "input": {
    ...
    "shareSyncSpaceName": null,
    "shareSyncBaseUrl": "https://your-org.files.sparkcloud.studio/dav/spaces/<id>/Spark%20Fuse%20Job%20Inputs/<jobId>/",
    "uploadUrl": "https://your-org.files.sparkcloud.studio/dav/spaces/<id>/Spark%20Fuse%20Job%20Inputs/<jobId>/spark-input.tar.gz",
    "uploadMethod": "PUT",
...
Spark Fuse waits up to **30 minutes** (the platform default; an
operator-tunable window) for the upload to land. Once it does, it pulls
the tarball into `/input/` and starts your container. Inside the
container, your files are at `/input/...` exactly as you laid them out
under the directory you ran `tar` from.

You can start the `PUT` any time after you get the `jobId` — the upload
can run while the job is still `queued`, and the tarball just waits on
ShareSync until Spark Fuse picks it up. The wait clock starts **after**
your image has been pulled onto the instance, not at submit.

If the upload hasn't fully landed by the end of the window the job fails
with `error_code='input_download_failed'`.

> **Billing warning — you pay for the wait.** The instance is already
> provisioned and **billing** while Spark Fuse waits for your upload.
> A failed or stalled upload that runs out the full window burns up to
> 30 minutes of compute before the job fails. If you realize the upload
> won't make it (e.g. a slow or dropped home uplink), **cancel the job**
> with `POST /api/compute/jobs/:jobId/cancel` rather than letting it run
> to timeout — cancel releases the compute immediately.

> **Large or reusable inputs? Use the mount workflow instead.** The
> push workflow is best for small, per-job inputs (a few MB of source).
> For tens of GB — model libraries, datasets, anything you'd rather not
> push over your uplink on every run — stage the files in ShareSync once
> and use the [mount workflow](#32-mount-workflow-pre-populated-input-folder)
> (§3.2): you submit a path that already exists, so there's no
> post-submit upload race and no timeout-billing exposure. See §3.2.

#### When `uploadUrl` is missing

If Spark Fuse couldn't compose the upload URL, the `input` block comes
back with `shareSyncBaseUrl: null`, no `uploadUrl` / `exampleCurl`, and a
`shareSyncUrlError` object telling you why:
...
```json
{
  "input": {
    ...
    "shareSyncUrlError": {
      "code": "unauthorized",
      "message": "ShareSync rejected the request (401)."
    }
  }
}
```
...
|------|---------|------------|
| `unauthorized` | ShareSync rejected your credentials (401/403). | Mint a fresh Spark token and retry; contact ops if it persists. |
| `project_not_found` | The ShareSync Project named in `inputShareSyncSpaceName` doesn't exist or you can't access it. | Fix the project name (or omit it to use your Personal space). |
| `graph_error` / `network_error` | A transient ShareSync issue. | Re-fetch with `GET /api/compute/jobs/{jobId}` (see below) — it's retried server-side first. |
| `deployment_missing` | Your org has no ShareSync file server. | Contact ops. |

The job is still created (you get a `jobId`). While it's `queued` or
`provisioning`, `GET /api/compute/jobs/{jobId}` re-composes the upload
URL, so once the underlying cause clears (e.g. you minted a new token)
you can recover the `uploadUrl` from the GET response without
resubmitting — as long as you upload before the wait window closes.

### 3.2 Mount workflow (pre-populated input folder)

Use this when your data **already lives in ShareSync** — the opposite of
the push workflow (§3.1), where you upload after submitting. You don't
get (or need) an `uploadUrl` here; you just tell Spark Fuse the path and
it pulls the contents into `/input/` before your command runs. This is
the recommended path for large or repeatedly-used inputs (model
libraries, datasets) because you stage the data once and never re-upload
it.

> **Naming note:** despite "mount," this does not lazily FUSE-mount the
> folder — at job start Spark Fuse **copies** everything under the path
> into a fresh read-only `/input/` for that job. Files are copied as-is;
> archives are **not** auto-extracted (that only happens for the push
> workflow's `spark-input.tar.gz`).
...
#### How to get files into ShareSync in the first place

The mount workflow assumes your files already live in ShareSync. There
are two ways to put them there — you only stage them once, then reference
the same path from as many jobs as you like.

**Option A — the Spark / ShareSync desktop app (easiest).** Download and
install the desktop app from
[sparkcloud.studio/downloads](https://sparkcloud.studio/downloads)
(Windows and macOS) and it surfaces ShareSync as a synced drive in
**Finder** (macOS) or **File Explorer** (Windows). Drag your model
library or dataset into a folder there and let it sync up. Whatever
folder you drop it in is the path you pass as `inputShareSyncPath` (e.g. a
folder you see as `My Stuff/models/` maps to
`inputShareSyncPath: "/My Stuff/models/"`). This is the right choice for
large, rarely-changing libraries you stage from your workstation.

**Option B — direct WebDAV upload (for tooling / headless).** ShareSync
is a WebDAV server, so you can create folders and upload files with plain
HTTP using your Spark token — the same auth and host as the §3.1 push
`PUT`, just to a path you choose. Create the directory with `MKCOL`, then
`PUT` each file:

```bash
# Base WebDAV URL for your space (see note below on how to obtain it):
export SS_BASE="https://your-org.files.sparkcloud.studio/dav/spaces/<id>"

# Create the destination folder (idempotent; 405/409 if it already exists):
curl -X MKCOL -H "Authorization: Bearer $SPARK_TOKEN" "$SS_BASE/models/"

# Upload a file into it:
curl -X PUT --data-binary @./sd_xl_base_1.0.safetensors \
  -H "Authorization: Bearer $SPARK_TOKEN" \
  "$SS_BASE/models/sd_xl_base_1.0.safetensors"
```

Then submit with `inputShareSyncPath: "/models/"`. For nested trees,
`MKCOL` each parent level before `PUT`ing files into it (WebDAV does not
create intermediate collections implicitly).

> **Getting your WebDAV base URL:** call
> [`GET /api/sharesync/location`](#34-resolving-your-sharesync-location)
> to resolve your files host + Personal-space WebDAV base URL directly,
> and [`GET /api/sharesync/projects`](#34-resolving-your-sharesync-location)
> to list your Projects with their Project IDs (§3.4). No prior job
> needed. (The base URL is also still surfaced on a job response as
> `input.shareSyncBaseUrl` — see §3.1.)

Pick whichever fits: drag-and-drop via the desktop app for one-time
staging from your machine, or the WebDAV flow for automated pipelines.
Either way, once the data is in ShareSync you get the "stage once,
reference everywhere" workflow — every job just points at the path.
...
### 3.4 Resolving your ShareSync location

So your tooling can target ShareSync paths without first running a job
and scraping `input.shareSyncBaseUrl`, two read-only endpoints resolve
your ShareSync location directly. Both take your normal bearer token.

These are **generic ShareSync endpoints** (under `/api/sharesync`, not
`/api/compute`) — they work for any Spark user with ShareSync, independent
of Spark Fuse; they're documented here because Fuse input staging is a
common reason to call them.

**Your files host + Personal space:**

`GET /api/sharesync/location`

```bash
curl -s "$SPARK_HOST/api/sharesync/location" \
  -H "Authorization: Bearer $SPARK_TOKEN" | jq '.'
```

```json
{
  "filesHost": "your-org.files.sparkcloud.studio",
  "personal": {
    "id": "<id>",
    "name": "Personal",
    "webDavBaseUrl": "https://your-org.files.sparkcloud.studio/dav/spaces/<id>/"
  }
}
```

**Your Projects (with their Project IDs):**

`GET /api/sharesync/projects`

```bash
curl -s "$SPARK_HOST/api/sharesync/projects" \
  -H "Authorization: Bearer $SPARK_TOKEN" | jq '.'
```
  ...
  "projects": [
    {
      "id": "<project-id>",
      "name": "Renders 2026",
      "webDavBaseUrl": "https://your-org.files.sparkcloud.studio/dav/spaces/<project-id>/"
    }
  ]
}
```

`webDavBaseUrl` is exactly the base you `MKCOL` / `PUT` against when
staging files (§3.2). To reference a Project on a job, pass its `name`
as `inputShareSyncSpaceName` (the API resolves the name for you) — the
`projects` list is there so your tooling can discover the available
Project names/IDs and build WebDAV URLs without a prior job.

| HTTP | Meaning |
|------|---------|
| 200  | Location / Projects resolved. |
| 403  | Token invalid. |
| 502  | ShareSync (Graph) was unreachable or rejected the request — retry; if it persists, contact ops. |

### 3.5 Read-only asset library (`/assets`)

For a **large, stable library you reuse across many jobs** — model
weights, LoRAs, a ComfyUI `models/` tree, reference datasets — use
`assetsShareSyncPath`. The platform mounts that ShareSync path
**read-only and lazily** at `/assets` inside your container and **caches
it on the compute node across jobs**, so a later job that reads the same
library starts from local disk instead of re-downloading.

This is different from `/input`:

| | `/input` (§3.1 / §3.2) | `/assets` (this section) |
|---|---|---|
| Purpose | The per-job inputs that change each run | A stable library reused across runs |
| Transfer | Copied in full into `/input/` before the job starts | Mounted; only the bytes you actually read are pulled |
| Across jobs | Re-fetched every run | Cached on the node — a repeat run reads from local disk |
| Mode | read-only | read-only |

They're independent and **combinable**: pass `inputShareSyncPath` for the
small changing inputs *and* `assetsShareSyncPath` for the big stable
library in the same job.

**Stage once, reference everywhere.** Put the library on ShareSync one
time (§3.2 covers the staging paths — the desktop app or a direct WebDAV
upload), then pass its path on every job:

```json
{
  "image": "your-registry/comfyui:latest",
  "command": ["python", "main.py"],
  "assetsShareSyncPath": "/My Stuff/comfyui-models/"
}
```

Your command reads the library at `/assets` (also exposed as the
`SPARK_ASSETS_DIR` environment variable). It is read-only — your job
cannot modify the library mid-run; write outputs to `/output` as usual.

To reference a library that lives in a **Project** (not your Personal
space), pass the Project name as `assetsShareSyncSpaceName` (same
resolution as `inputShareSyncSpaceName`; see §3.4 to list your Projects).

Spark Fuse **PROPFIND-probes** `assetsShareSyncPath` for read access at
submit time and rejects with HTTP 400 if the path is unreachable, so a
typo fails fast instead of mid-job.

**First run vs. warm node.** The first job to read the library on a given
node pulls bytes on demand (slower that once); subsequent jobs that land
on the same warm node read from the local cache (fast). Your job log
prints which case applies when the mount comes up. This pairs naturally
with cached-image affinity (§2.7): a warm node with both your image and
your asset cache makes repeated cold-starts dramatically faster.

| Request field | Type | Required | Notes |
|---|---|---|---|
| `assetsShareSyncPath` | string | no | ShareSync path to mount read-only + lazily at `/assets`. Omit for no assets mount. |
| `assetsShareSyncSpaceName` | string | no | Project the path lives in. Omit = your Personal space. |

| 400 error | Fix |
|---|---|
| `assetsShareSyncPath must start with '/' …` | Pass a logical ShareSync path beginning with `/`. |
| `assetsShareSyncPath validation failed (…)` | Verify the path exists and you have read access. |
| `assetsShareSyncSpaceName validation failed (project_not_found)` | Check the Project name (see §3.4). |
...
```json
{
  ...
  "log_archive_share_sync_path": "/Spark Fuse Job Outputs/16fb6d80-.../spark-fuse-16fb6d80-....log",
  ...
  "image_affinity": "preferred",
  "imageAffinity": "preferred",
  "image_cache_hit": true,
  "imageCacheHit": true,

  "output": {
    "shareSyncPath": "/Spark Fuse Job Outputs/16fb6d80-.../",
    "shareSyncSpaceName": null,
    "shareSyncBaseUrl": "https://your-org.files.sparkcloud.studio/dav/spaces/<id>/Spark%20Fuse%20Job%20Outputs/16fb6d80-.../"
  },
  "input": null
...
returns the job row unchanged. Cancelling a queued job takes it out of the
queue within a few seconds — before any compute is started — so it never
incurs runtime. Cancelling a running job sends a SIGTERM to the
...
> `**outputShareSyncPath` defaults to `/Spark Fuse Job Outputs/{jobId}/**` — if you
...
>
> **Default folder rename note:** the default top-level folder has been
> renamed twice. It started as `/Compute Jobs/{jobId}/`, became
> `/Spark Fuse Job Outputs/{jobId}/` (2026-05-06, to match the Spark Fuse
> product brand), and is now `/Spark Fuse Job Outputs/{jobId}/`
> (2026-06-02, so the output default visually pairs with the
> auto-prepare input default `/Spark Fuse Job Inputs/{jobId}/`). All
> renames are same-shape — only the top-level folder name changed. If you
> have past jobs whose archives landed in an older folder, those archives
> are still there and still listed verbatim on each job's
...
# ---- 1. Auth ---------------------------------------------------------
LOGIN_RESPONSE=$(curl -sX POST "$SPARK_HOST/auth/login" \
  -H "Content-Type: application/json" \
  -d "{
    \"email\": \"$SPARK_EMAIL\",
...
SUBMIT_RESPONSE=$(curl -sX POST "$SPARK_HOST/api/compute/jobs" \
  ...
  -d "{
    ...
    \"inputPushMode\": \"auto-prepare\",
    \"outputShareSyncPath\": \"/Spark Fuse Job Outputs/end-to-end-example/\"
  }")
...
| `input_download_failed`            | (auto-prepare) Your upload didn't fully land within the wait window (30 min default), or the upload was malformed. For large inputs, use the mount workflow (§3.2) instead.                                                                                                                                                                                                                                                                                                                                      |
...
| `registry_auth_failed`                     | (Terminal) The compute agent's `docker login` against the customer's private registry returned non-zero — typically the credential's token was revoked, rotated externally, or expired between submission-time validation and the actual provision. Rotate the token via `PUT /api/compute/credentials/:credentialId/secret` and resubmit. See §2.6. |
| `credential_decryption_failed`             | (Terminal) The compute agent could not decrypt the credential secret shipped in the jobSpec. This is a platform misconfiguration, not a customer error — contact Spark Cloud Studio support. See §2.6. |
| `credential_not_found`                     | (Terminal) The credential referenced by the job row's `registryCredentialName` was deleted between submission and provisioning, so the job spec assembler couldn't attach it. Re-create the credential with the same name (or pick a different one) and resubmit. See §2.6. |
...
```typescript
{
  ...
  output: {
    ...
    shareSyncBaseUrl: string | null;  // null on ShareSync resolve failures
    // Present only when shareSyncBaseUrl is null due to a resolution
    // failure (see "When uploadUrl is missing" in §3.1).
    shareSyncUrlError?: {
      code: 'unauthorized' | 'project_not_found' | 'graph_error'
          | 'network_error' | 'deployment_missing';
      message: string;
    } | null;
  };
  input: {
    ...
    uploadMethod?: 'PUT';
    exampleCurl?: string;
    // Same shape/semantics as output.shareSyncUrlError above. For an
    // auto-prepare job a non-null value is why uploadUrl is absent.
    shareSyncUrlError?: {
      code: 'unauthorized' | 'project_not_found' | 'graph_error'
          | 'network_error' | 'deployment_missing';
      message: string;
  ...
  // Cached-image affinity (§2.7). Echo of the requested affinity
  // ('preferred' | 'required') and the placement outcome:
  // imageCacheHit = true (cache hit, fast start), false (cold pull on
  // a fresh node), or null (not yet placed / N/A). Both also returned
  // snake_case as image_affinity / image_cache_hit.
  imageAffinity: 'preferred' | 'required';
  imageCacheHit: boolean | null;
}
```
...
Cached-image affinity (§2.7) is likewise returned in both forms:
`image_affinity` / `imageAffinity` (the requested preference) and
`image_cache_hit` / `imageCacheHit` (the placement outcome — `true`
cache hit, `false` cold pull, `null` while still `queued`).
...
| `POST`   | `/api/compute/credentials/registry`              | [Create a private-registry credential (Docker Hub / GHCR)](#26-private-container-images). Org admins only. |
| `GET`    | `/api/compute/credentials`                       | [List your organisation's credentials](#26-private-container-images). Org admins only. |
| `PUT`    | `/api/compute/credentials/:credentialId/secret`  | [Rotate a credential's secret in place](#26-private-container-images). Org admins only. |
| `DELETE` | `/api/compute/credentials/:credentialId`         | [Delete a credential](#26-private-container-images). Org admins only. |
| `GET`    | `/api/compute/credentials/:credentialId/audit`   | [Audit trail for a credential](#26-private-container-images). Org admins only. |
