...
capacity is currently capacity-limited for some GPU SKUs at peak
times — see the callout at the top of §13. Multi-region SmartCompute
is rolling out next; once it lands, the same `mode='smart'` API
contract will benefit from a much wider capacity pool without any
client changes.
...
```typescript
{
  ...
  // default (compute_settings.compute_container_shm_size, currently
  // '2g'), or the hardcoded '2g' fallback when neither is available.
  shmSize?: string;
}
```
...
convenience: `container_shm_size` (snake_case row column, always
present) and `shmSize` (camelCase alias, always present). Both carry
the same value.
...
> **Capacity availability — v1.15 limitation, lifting with the
> multi-region SmartCompute rollout.** SmartCompute capacity for some
> GPU SKUs can be **extremely limited** at peak times in v1.15.
...
>
> The next Spark Fuse release widens SmartCompute capacity across
> additional compute locations and is expected to alleviate this.
> The API contract for `mode='smart'` does not change, so anything
> you build against this section will continue to work once capacity
> widens — there is nothing to opt into on the client side.
...
batch path. The multi-region SmartCompute rollout (next release)
widens this without any client-side changes.
...
*Last updated 2026-05-19 for Spark Fuse v1.15 (current release). Tracks
API v1.0 + v1.1 (q) SmartCompute + v1.15 (b) smart-mode estimate
pricing. Includes the v1.15 SmartCompute capacity caveat (§0.1 / §13 /
§13.4); the multi-region SmartCompute rollout will lift that caveat
without changing the `mode='smart'` API contract. Vocabulary
harmonized with the ComfyUI headless quickstart (compute/GPU/warm-
member language; no underlying-provider references).
...
- Removed `availabilityRegion` from the customer-facing submit
  ...
  example ([§5](#5-polling-job-status)), and the v1 region limits
  call-out ([§11.8](#118-limits--quotas-v1)) ahead of the
  multi-region SmartCompute rollout. Region placement is now
...
- New optional submission field `shmSize` (Docker `--shm-size` override;
  platform default `2g`, max `32g`) — see §2.2 + §2.3 + §11.4 + §11.5.
