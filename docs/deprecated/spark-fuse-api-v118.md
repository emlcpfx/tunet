# Spark Fuse API — customer guide (v1.18)
...
| `maxWallClockSeconds`        | number   | no       | **Opt-in only.** Hard ceiling on container wall-clock runtime, in seconds. Omit (the default) and Spark Fuse will **never** terminate your job on time-elapsed alone — your container runs until it exits, you cancel it, or the inactivity detector fires (see `containerInactivitySeconds` below). Supply an integer to opt in: when the container has been running for this many seconds, the platform SIGKILLs it and the job lands terminal `failed` with `errorCode='wallclock_exceeded'`. Range `[60, 86400]` (1 min to 24 h; the upper bound is hot-tunable by Spark ops if your workload legitimately needs longer). Distinct from `containerInactivitySeconds`, which fires when your container is making no progress regardless of how long it has been running. Echoed back on every job response as `maxWallClockSeconds` (snake_case alias `max_wallclock_seconds`). |
| `containerInactivitySeconds` | number   | no       | Per-job override for the platform's container-inactivity detector. Inactivity = **no** stdout/stderr lines emitted **AND** **no** CPU activity **AND** **no** GPU activity for the full window. When all three signals are quiet, the platform SIGTERMs your container (30 s grace, then SIGKILL) and the job lands terminal `failed` with `errorCode='container_inactive'`. The three-signal AND is intentional — training loops that don't log but keep the GPU busy do **not** trigger the detector. **Tri-state**: omit (the default) to use the platform default (currently **1800 s = 30 min**, tunable by Spark ops via the parameter map); pass `0` to **disable** the detector entirely for this job (use sparingly — for legitimately long-quiet batch pipelines); pass a positive integer to override the default with your own threshold. Range `[60, 86400]` for positive values, or exactly `0`. Echoed back on every job response as `containerInactivitySeconds` (snake_case alias `container_inactivity_seconds`). |
...
| 400 `error_code='invalid_container_inactivity_seconds'`           | `containerInactivitySeconds` was supplied but isn't an integer that is either exactly `0` (disable the detector) or in `[60, 86400]` (override the platform default). Omit the field entirely to use the platform default.                                                                                                                                                                                                                                            |
...
| `output_upload_failed`             | Outputs couldn't be uploaded to ShareSync. The container may have produced valid outputs that simply didn't make it back. Retained for older job submissions; new submissions surface the `output_mount_*` codes below instead — see §13.6.                                                                                                                                       |
| `output_mount_failed`              | The platform couldn't bring up the `/output/` mount before the container started. The container never ran — semantically equivalent to a provisioning fault. Not customer-actionable; resubmit. See §13.6. |
...
| `output_unmount_failed`            | A rare platform-side teardown failure releasing the per-job `/output/` mount after the container exited. Appended to the underlying terminal status rather than replacing it. Not customer-actionable; resubmit if any output files appear missing. See §13.6. |
| `container_nonzero_exit`           | Your command exited with a non-zero status. `exit_code` field carries the exit code.                                                                                                                                                                                                                                                                                                                                                  |
| `cancelled`                        | The job was cancelled via `POST /api/compute/jobs/:id/cancel` while running. `status` is projected to `cancelled` for cancel-driven failures — see §11.1.                                                                                                                                                                                                                                                                                                                                 |
...
```typescript
{
  ...
  // the submission omitted the field — the platform will NEVER kill
  ...
  // `null` = platform default applies; `0` = customer disabled the
...
sentinel — the platform never kills a job on time-elapsed alone
unless this field carries a positive integer.
...
- The local cache is sized per-job (a few GiB by default). If your
  workload writes faster than the platform can flush back to
  ShareSync, the platform applies **backpressure** — `write()`
  syscalls block — rather than silently dropping data. A container
  that's blocked on backpressure looks no different from one
  that's blocked on slow local-disk I/O.
...
or a SmartCompute preemption notice fires, the platform waits up to
a short drain window (around a minute) for any in-flight writes to
land in ShareSync before reporting the terminal status. Three
observable outcomes:
...
3. **Unmount failure:** rare — the platform couldn't release the
   per-job `/output/` mount after the container exited. Surfaces as
...
**Pre-job mount failure.** If the platform can't bring up the
`/output/` mount in the first place, the container never runs and
the job lands terminal with `error_code='output_mount_failed'`.
Semantically equivalent to a provisioning fault; SmartCompute retry
budgets do not treat this as a preemption (no retry credit
consumed).
