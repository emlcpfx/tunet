# Spark Fuse API — customer guide (v1.17)
...
> **Region placement is automated.** Spark Fuse picks where each job
> runs for you — both InstantCompute and SmartCompute. There is no
> region knob on the API surface, and cost quotes are region-agnostic
> (the platform quotes against the worst-case rate across every
> compute location the SKU is offered in, then picks an actual
> placement at launch time). Older clients that still send an
> `availabilityRegion` field on submit or estimate calls continue to
> work — the value is accepted on the wire but ignored.
...
Every submit is a fresh cold-start — no warm pool. See
[§13](#13-smartcompute-mode-smart) for the deep-dive (retry budget,
preemption webhook, trade-offs). SmartCompute capacity is currently
capacity-limited for some GPU SKUs at peak times — see the callout
at the top of §13.
...
| `notifyOnFailure`          | boolean  | no       | Per-job override for the failure email — see [§13.7](#137-interruption-no-retries-left-email-notifications). Tri-state: omit (or pass `null`) to use the platform default for your org; pass `true` to force-send so this job emails even if the platform default is off; pass `false` to force-suppress so this job doesn't email even if the platform default is on. Echoed verbatim on every job response. **Current scope:** the only failure that triggers an email today is the smart-mode `smartcompute_interrupted_no_retries_left` terminal — in-container failures (nonzero container exit, image-pull errors, etc.) do not email regardless of this field. |
...
| 503                                                               | All warm members are busy AND no eligible warm member could be reshaped in time. Retry after a few seconds. (Rare.) |
...
| `mode`                    | string | no       | `instant` (default) or `smart`. Both modes quote against the worst-case hourly rate currently observed across every compute location the SKU is available in — the platform picks the actual landing location at launch time, so quoting the high end is honest. Real-world billed cost is typically lower. `rate.billedPerHourUsd` is the final customer rate for the requested mode — read it as-is. If smart-mode pricing data isn't yet observed for the SKU (rare; happens for a brief window after a new SKU is added to the catalog), smart-mode falls back to a discount-against-instant approximation.    |
...
> exists for every SKU at every moment. If a submission you'd expect to
> succeed times out waiting on capacity, retry with `g6` / `g7e` family
> alternates.
...
| `output_upload_failed`             | Outputs couldn't be uploaded to ShareSync. The container may have produced valid outputs that simply didn't make it back. (Legacy `scratch_then_upload` strategy only — superseded by the three `output_mount_*` codes below once the platform default flips to the rclone-FUSE mount; see §13.6.)                                                                                                                                       |
| `output_mount_failed`              | (rclone-FUSE strategy only) The agent couldn't establish the per-job `/output/` FUSE mount before the container started. Typically a missing `rclone` / `fuse3` binary on the compute host, a Keycloak mint failure for the per-job bearer token, or the mount didn't appear in `/proc/mounts` within the wait window. The container never ran — semantically equivalent to a provisioning fault. See §13.6. |
| `output_drain_failed`              | (rclone-FUSE strategy only) The container ran to its terminal state, but the post-exit drain budget (default 60s, tunable via the `compute_output_mount_drain_budget_seconds` platform setting) expired with pending VFS-cache writes still in flight to ShareSync. Some `/output/` files may not have reached ShareSync. Preserves the underlying terminal status (`succeeded`, `failed`, `smartcompute-interrupted`) by appending the code rather than replacing it. See §13.6. |
| `output_unmount_failed`            | (rclone-FUSE strategy only) The per-job `/output/` mount could not be released even after escalating to `fusermount -uz` and `sudo umount -l`. The host scratch dir is left in place for operator cleanup. Appended to the underlying terminal status rather than replacing it. Indicates a kernel-level FUSE failure on the compute host; rare and typically not customer-actionable. See §13.6. |
...
```typescript
{
  ...
  // e.g. '2g'). Echo of either the submitted shmSize, the platform
  // default (currently '2g'), or the hardcoded '2g' fallback when
  // neither is available.
  shmSize?: string;

  // Echo of the resolved per-job notifyOnFailure override (§2.2 +
  // §13.7). `null` when the submission omitted the field (defer to
  // the platform default); `true` / `false` when an explicit value
  // was supplied.
...
### 11.7 Spark Fuse is for Linux containers only

Spark Fuse runs Linux Docker containers on Linux hosts. There is no
Windows-compute path. The image you supply must be linux/amd64 (or
linux/arm64 for ARM-eligible SKUs once those are added).
...
than infinite retry.
- **Region placement:** automated by the platform — there is no
customer-controllable region knob. Any `availabilityRegion` field
passed on submit or estimate is accepted on the wire (for backward
compatibility with older clients) but ignored.
...
| Cold-start            | ~2 s on warm hit; ~3 min if reshape                | ~3 min cold provisioning every time (no warm pool)                                |
...
interruptions — see [§13.5.1](#1351-retry_history-records) and
[§13.5.2](#1352-pre-billing-cost-vs-actual-billed-cost).
...
different compute location to maximize fulfillment chances). When
...
in (the platform picks the actual landing location at launch, so
quoting the high end is honest — real-world billed cost is typically
lower). `rate.billedPerHourUsd` is the final customer rate for the
...
the top of §13. Confirm the SKU you want is finding capacity in
...
`output.shareSyncPath` is reused across retries within the same
`jobId`).
...
| `retry_history`                              | array         | Per-attempt audit trail. Append-only, ordered by attempt. Each element carries `attemptNumber` (1-indexed), `awsInstanceId`, `startedRunningAt`, `terminalAt`, `terminalStatus`, `errorCode`, `attemptedComputeCostUsdEstimate`, `source`. See [§13.5.1](#1351-retry_history-records) below. |
| `total_attempted_compute_cost_usd_estimate`  | string | null | Rolling pre-billing sum of `retry_history[*].attemptedComputeCostUsdEstimate`. `null` until the first attempt completes. NOT an authoritative billed amount — see [§13.5.2](#1352-pre-billing-cost-vs-actual-billed-cost). |
...
| `awsInstanceId`                    | string | null | EC2 instance id this attempt ran on. `null` for attempts that never bound to an instance (queue-time cancels, queue-insert failures).                              |
...
| `errorCode`                        | string | null | The `error_code` the attempt ended with. Mirrors the customer-facing values in [§11.2](#112-error-codes).                                                          |
...
#### 13.5.2 Pre-billing cost vs actual billed cost
...
### 13.6 Retry-safe containers: `/output/` persistence + the checkpoint pattern

Spark Fuse exposes `/output/` inside your container as a **live
WebDAV-backed FUSE mount** of the job's ShareSync output collection.
...
retry attempt of the same `jobId`, so partial work from a preempted
...
- Write latency to `/output/` is local-disk fast (rclone's VFS
  cache absorbs writes synchronously to local NVMe; flushes back
  to ShareSync happen asynchronously on a configurable interval,
  default 10s — controlled by the platform setting
  `compute_output_mount_writeback_interval_seconds`).
- The VFS cache is sized per-job (default 4 GiB, tunable via
  `compute_output_mount_cache_max_size_mib`). If your workload
  writes faster than rclone can flush back to ShareSync, rclone
  applies **backpressure** — `write()` syscalls block — rather
  than silently dropping data. A container that's blocked on
  backpressure looks no different from one that's blocked on
  slow local-disk I/O.
- Read latency is roughly the same as `/input/`: files already
  present in the cache (e.g. ones the same container just wrote)
  return at NVMe speed; cold reads fetch from ShareSync once and
  then cache.

**End-of-job drain.** When your container exits (zero or non-zero)
or a SmartCompute preemption notice fires, the agent sends rclone a
graceful shutdown and waits up to the configured drain budget
(default 60s, tunable via `compute_output_mount_drain_budget_seconds`)
for any in-flight writes to land in ShareSync before reporting the
terminal status. Three observable outcomes:
...
3. **Unmount failure:** rare — the post-drain unmount itself
   couldn't release the mount point. Surfaces as
   `error_code='output_unmount_failed'` (appended, like
   `output_drain_failed`). Indicates a kernel-level FUSE fault on
   the host; the platform leaks the host scratch dir for operator
   cleanup rather than risk recursing into a half-mounted FUSE
   tree.

**Pre-job mount failure.** If the agent can't bring up the mount in
the first place (missing `rclone` / `fuse3` binary on the compute
host, Keycloak mint failure for the per-job bearer token, mount
didn't appear in `/proc/mounts` within ~30s), the container never
runs and the job lands terminal with
`error_code='output_mount_failed'`. Semantically equivalent to a
provisioning fault; SmartCompute retry budgets do not treat this
as a preemption (no retry credit consumed).
...
- `/output/` is a live FUSE mount of the ShareSync collection;
...
- `retries_used_on_interrupt` / `max_retries_on_interrupt` and the
  ...
  cost-estimate from [§13.5](#135-smartcompute-retry-visibility)).
...
- A link to the partial output collection on ShareSync (the
  ...
  per [§13.6](#136-retry-safe-containers-output-persistence--the-checkpoint-pattern)).

Controlling the email per job:

Pass the optional `notifyOnFailure` field on submit to override the
platform default for a single job. The override only affects the
event described above — the `smartcompute_interrupted_no_retries_left`
terminal on a smart-mode job. In-container failures (nonzero
container exit, image-pull errors, agent-silent watchdog, etc.) do
not email today regardless of `notifyOnFailure`; the field also has
no effect on `mode='instant'` jobs because they can't hit this
event in the first place.

- `notifyOnFailure: true` — force-send. The email fires even if
  the platform default for your org is "off". Use when you want
  the email for a particular run even if your org's default is
  suppressed.
- `notifyOnFailure: false` — force-suppress. The email does not
  fire even if the platform default is "on". Use for batch
  submissions where you're already polling for terminal state and
  don't want one email per job.
- Omit the field (or pass `null`) — defer to the platform default
  for your org. This is the existing behaviour.

The resolved value is echoed back on the submit response (as
`notifyOnFailure`) and on every GET / list / cancel response (as
the snake_case row column `notify_on_failure`) so you can confirm
what the platform stored.

### 13.8 Smart-mode end-to-end (curl)
...
2026-05-20 revisions:
- New optional submission field `notifyOnFailure` (tri-state
  `boolean | null`) on POST `/api/compute/jobs` for per-job control
  of the [§13.7](#137-interruption-no-retries-left-email-notifications)
  failure email. Omit to use the platform default for your org
  (existing behaviour); pass `true` to force-send for this job
  regardless of the platform default; pass `false` to force-suppress
  regardless of the platform default. Echoed back on every job
  response. **Scope:** the override only affects the smart-mode
  `smartcompute_interrupted_no_retries_left` terminal email — the
  only failure mode currently wired to email at all. In-container
  failures (nonzero exit, image-pull errors, etc.) do not email
  today regardless of this field. See
  [§2.2](#22-submit-dto--full-field-list),
  [§5](#5-polling-job-status), and
  [§13.7](#137-interruption-no-retries-left-email-notifications).

2026-05-19 revisions:
- Removed `availabilityRegion` from the customer-facing submit
  ([§2.2](#22-submit-dto--full-field-list)) and estimate
  ([§2.8](#28-cost-estimation-pre-submit)) DTOs, the GET response
  example ([§5](#5-polling-job-status)), and the region limits
  call-out ([§11.8](#118-limits--quotas)). Region placement is now
  automated by the platform across both modes. Older clients that
  still send `availabilityRegion` on submit / estimate continue to
  work — the value is accepted on the wire but ignored, so existing
  integrations do not need to be updated.
- Smart-mode estimate language updated to talk about "every compute
  location" rather than region/zone, matching the new region-agnostic
  worst-case-rate quoting semantics (§2.8, §13.3, §13.4).

2026-05-18 revisions:
- New optional submission field `shmSize` (Docker `--shm-size` override;
  platform default `2g`, max `32g`) — see §2.2 + §2.4 + §11.4 + §11.5.
  Fixes the PyTorch DataLoader `num_workers > 0` `Bus error` against
  Docker's built-in 64 MB `/dev/shm` default.
- Estimate response `rate` block is `{ billedPerSecondCents,
  billedPerHourUsd }` — the final customer rate, read as-is. `notes[]`
  carries the runtime-vs-estimate caveat (plus smart-mode-specific
  entries when `mode='smart'`). See §2.8.*